...
with lesson as (
  ...
  values (
    (select id from public.topics t join public.curricula c on c.id = t.curriculum_id
...
with lesson as (
  ...
  values (
    (select id from public.topics t join public.curricula c on c.id = t.curriculum_id
     where c.subject = 'Mathematics' and t.class_level = 'SS2' and t.term = 2 and t.order_index = 211),
