    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-amber-500 selection:text-slate-950">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12 w-full flex-grow">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-display font-extrabold text-white">Join Your School</h1>
        ...
        {step === 1 && (
          <Card variant="paper" className="p-8">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-800">
              ...
              <div>
                <h2 className="text-lg font-display font-bold text-white">Search for your school</h2>
                <p className="text-xs font-mono text-slate-400">Search first — creating a duplicate is blocked, so check here before adding a new one</p>
              </div>
            </div>
            ...
            {searched && (
              <div className="mt-6 space-y-2 font-mono">
                {results.length === 0 && (
                  <p className="text-xs text-slate-400 mb-3">No matching school found.</p>
                )}
                {results.map((s) => (
                  <div key={s.id} className="flex items-center justify-between p-3.5 rounded-xl bg-slate-900 border border-slate-800">
                    <div>
                ...
                <button
                  type="button"
                  onClick={() => setStep(2)}
                  className="w-full text-center text-xs font-mono text-amber-400 hover:underline pt-3"
                >
        ...
        {step === 2 && (
          <Card variant="paper" className="p-8">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-800">
              ...
              <div>
                <h2 className="text-lg font-display font-bold text-white">Add your school</h2>
            ...
            <form onSubmit={handleCreate} className="space-y-4 font-mono">
              <div>
                <label className="text-xs font-bold text-slate-300 block mb-2">School Name</label>
                <input
                  type="text"
                  value={newName}
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 text-xs text-white focus:outline-none focus:border-amber-500"
                >
              ...
              <div>
                <label className="text-xs font-bold text-slate-300 block mb-2">Address (optional)</label>
                <input
                  type="text"
                  value={newAddress}
          ...
          <Card variant="paper" className="p-8 text-center">
            {outcomeSchool.status === 'active' ? (
              <>
                ...
                <h2 className="text-2xl font-display font-extrabold text-white">You&apos;re linked to {outcomeSchool.name}</h2>
              ...
              <>
                ...
                <h2 className="text-2xl font-display font-extrabold text-white">Suggestion submitted</h2>
                <p className="text-slate-300 text-xs font-mono mt-2 max-w-md mx-auto">
                  <strong className="text-amber-400">{outcomeSchool.name}</strong> is awaiting admin review. You&apos;ll be able to join it automatically once approved.
                </p>
              </>
            )}
