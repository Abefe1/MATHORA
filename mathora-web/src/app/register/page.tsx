...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex items-center justify-center px-4 py-12 font-sans bg-graph-paper">
      <motion.div
        ...
        <Card variant="notebook" className="shadow-2xl">
          {needsConfirmation ? (
            <div className="text-center py-4">
              ...
              <h1 className="font-display text-lg font-bold text-slate-900 dark:text-white mb-1">Check your inbox</h1>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                We sent a confirmation link to <span className="text-slate-800 dark:text-slate-200">{email}</span>. Confirm your
                email to finish creating your account.
              </p>
            </div>
          ) : (
            <>
              <h1 className="font-display text-xl font-bold text-slate-900 dark:text-white mb-1">Create your account</h1>
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-6">Join Mathora as a student, teacher, or parent.</p>

              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label htmlFor="fullName" className="block text-xs font-bold uppercase text-slate-500 dark:text-slate-400 mb-1.5">
                    Full Name
                  </label>
                  <input
                    id="fullName"
                ...
                <div>
                  <span className="block text-xs font-bold uppercase text-slate-500 dark:text-slate-400 mb-1.5">I am a…</span>
                      ...
                      <button
                        ...
                        className={`rounded-lg border px-2 py-2 text-xs font-bold transition-colors ${
                          role === r.value
                            ...
                            : 'bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white'
                        }`}
                      >
                        {r.label}
                      </button>
              ...
              <p className="text-center text-xs text-slate-500 dark:text-slate-400 mt-6">
                ...
                <Link href="/login" className="font-bold text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300">
                  Sign in
                </Link>
              </p>
            </>
