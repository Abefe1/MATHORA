  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-amber-500 selection:text-slate-950">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full flex-grow flex flex-col">
        {/* Top Header */}
        <div className="flex items-center justify-between gap-4 mb-6 bg-slate-900/80 p-5 rounded-2xl border border-slate-800 backdrop-blur-md">
          ...
          <span className="text-xs font-mono text-slate-400 hidden sm:inline-block">
            Connected as <strong className="text-amber-400">Chidiebere Okafor (SS2)</strong>
          </span>
        </div>
          ...
          <Card variant="paper" className="p-4 flex flex-col space-y-4 lg:col-span-1">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <span className="text-xs font-mono font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                <Users className="w-4 h-4 text-amber-400" /> Study Squads
              </span>
              ...
              {channels.map((ch) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold font-sans text-slate-100 flex items-center gap-1.5 truncate">
          ...
          <Card variant="paper" className="p-6 flex flex-col lg:col-span-3 space-y-4 min-h-[500px]">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-slate-800 font-mono">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-display font-bold text-white font-sans">{activeChannel.name}</h2>
                  <p className="text-xs text-slate-400">{activeChannel.description}</p>
                </div>
              </div>

              <span className="text-xs text-emerald-400 bg-emerald-950 px-2.5 py-1 rounded-md border border-emerald-800 flex items-center gap-1">
            ...
            <div className="flex-grow space-y-4 overflow-y-auto pr-2 font-mono">
              {activeChannel.messages.map((msg) => (
                <div key={msg.id} className="flex items-start gap-3">
                  ...
                  <div className="flex-grow bg-slate-900/90 border border-slate-800 rounded-2xl p-3.5">
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-slate-200 font-sans">{msg.senderName}</span>
                    ...
                    <p className="text-xs text-slate-300 leading-relaxed font-sans mt-1">{msg.content}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {/* Message Input Box */}
            <form onSubmit={handleSendMessage} className="pt-4 border-t border-slate-800 font-mono flex items-center gap-3">
              <input
                type="text"
                value={messageInput}
                onChange={(e) => setMessageInput(e.target.value)}
                placeholder={`Message #${activeChannel.name}...`}
                className="flex-grow bg-slate-900 border border-slate-800 rounded-xl px-4 py-3 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-amber-500 transition-colors"
              />
