    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col font-sans selection:bg-amber-500 selection:text-slate-950">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12 w-full flex-grow">
        ...
        <div className="text-center mb-8">
          <Link href="/parent" className="inline-flex items-center gap-1.5 text-xs font-mono text-amber-600 dark:text-amber-400 hover:underline mb-4">
        ...
        {step === 1 && (
          <Card variant="paper" className="p-8">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-200 dark:border-slate-800">
              ...
              <div>
                <h2 className="text-lg font-display font-bold text-slate-900 dark:text-white">Step 1: Student Phone or ID</h2>
                <p className="text-xs font-mono text-slate-500 dark:text-slate-400">Enter your child&apos;s registered phone number</p>
              </div>
            </div>

            <form onSubmit={handleStep1Submit} className="space-y-6 font-mono">
              <div>
                <label className="text-xs font-bold text-slate-600 dark:text-slate-300 block mb-2">Student Registered Phone Number</label>
        ...
        {step === 2 && (
          <Card variant="paper" className="p-8">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-200 dark:border-slate-800">
              ...
              <div>
                <h2 className="text-lg font-display font-bold text-slate-900 dark:text-white">Step 2: Enter Student PIN</h2>
                <p className="text-xs font-mono text-slate-500 dark:text-slate-400">Enter the 4-digit code shown on your child&apos;s app settings</p>
              </div>
            </div>

            <form onSubmit={handleStep2Submit} className="space-y-6 font-mono">
              <div>
                <label className="text-xs font-bold text-slate-600 dark:text-slate-300 block mb-2">4-Digit Security PIN</label>
                <input
                  type="password"
                  maxLength={4}
                  value={pinCode}
                  ...
                  placeholder="• • • •"
                  className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3 text-center text-lg tracking-widest text-amber-600 dark:text-amber-400 placeholder-slate-600 focus:outline-none focus:border-amber-500 transition-colors"
                />
              </div>
        ...
        {step === 3 && (
          <Card variant="paper" className="p-8">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-200 dark:border-slate-800">
              ...
              <div>
                <h2 className="text-lg font-display font-bold text-slate-900 dark:text-white">Step 3: Confirm Student Profile</h2>
                <p className="text-xs font-mono text-slate-500 dark:text-slate-400">Confirm student details before linking</p>
              </div>
            </div>

            <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 font-mono space-y-3 mb-6">
              <div className="flex items-center justify-between border-b border-slate-200/80 dark:border-slate-800/80 pb-2">
                ...
                <span className="text-sm font-bold text-slate-900 dark:text-white font-sans">{mockFoundStudent.name}</span>
              </div>
              <div className="flex items-center justify-between border-b border-slate-200/80 dark:border-slate-800/80 pb-2">
                <span className="text-xs text-slate-500 dark:text-slate-400">Class Grade</span>
                <span className="text-xs font-bold text-amber-600 dark:text-amber-400">{mockFoundStudent.class}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs text-slate-500 dark:text-slate-400">School</span>
                <span className="text-xs text-slate-600 dark:text-slate-300">{mockFoundStudent.school}</span>
              </div>
            </div>
        ...
        {step === 4 && (
          <Card variant="paper" className="p-8 text-center">
            ...
            <h2 className="text-2xl font-display font-extrabold text-slate-900 dark:text-white">Account Linked Successfully!</h2>
            <p className="text-slate-600 dark:text-slate-300 text-xs font-mono mt-2 max-w-md mx-auto">
              You are now connected to <strong className="text-amber-600 dark:text-amber-400">{mockFoundStudent.name}</strong>. Weekly progress summaries and teacher notes will automatically update in your Parent Corner.
            </p>
